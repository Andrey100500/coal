/* НАЗВАНИЕ: Система управления корзиной «Олимп» (v6.1 - Отправка заказа)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.submitOrderForm = function(event) {
        ...
        // Простая валидация телефона (можно улучшить под твой формат)
        ...
        const payload = {
            t: "Заказ с сайта",
            o: "Корзина", // Поле для совместимости
            items: itemsList + '\n\n' + totalInfo,
        ...
        // Отправляем данные
        window.sendData(payload, submitBtn);
     ...
     * Проверяет каждый товар на странице и вешает класс .in-cart, если он в корзине
     */
    ...
    const renderFullCartPage = () => {
        ...
        if (!tableBody) return; // Выходим, если мы не на странице cart.html
    ...
    const updateCartWidget = () => {
        ...
        updateButtonStates();
        renderFullCartPage(); // Синхронизируем страницу корзины, если мы на ней
    };
    ...
    const handleCartAction = async (productId, productName, categoryTag) => {
        ...
        if (isInCart) {
            // Если мы на странице cart.html — ничего не делаем, если нет — открываем модалку
    ...
    // Слушатель событий для кнопок "В корзину"
    ...
    // Глобальные методы управления количеством (для onclick в HTML)
    ...
    // Открытие модалки при клике на виджет
    ...
    // Вешаем обработчик на форму заказа, если она есть
    if (orderForm) {
